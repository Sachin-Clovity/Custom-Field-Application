...
//             onNext={handleTempfieldlateConfigNext}
//             onClose={onClose}
//             fieldName={fieldName}
//             fieldDesc={fieldDesc}
//           />
...
// this is working for multiple models now i am working for passing the value 


// import React, { useEffect, useState } from "react";
// import { invoke, view } from "@forge/bridge";   // ✅ Forge bridge
// import MoSCoWPrioritization from "./MoSCoWPrioritization";
// import TShirtSizeField from "./TShirtSizeField";
// import ScreenAssociation from "./ScreenAssociation";
...
//       const lid = ctx?.app?.localId || ctx?.localId;
//       setLocalId(lid || "");
//     });
//   }, []);

//   const TYPE_KEYS = {
//     "MoSCoW Prioritization": "fieldui-custom-field-ui-kit-2-hello-world",
//     "T-Shirt Size": "fieldui-custom-field-ui-kit-2-hello"
//     // add more templates here if you add more types
//   };

//   // Build ARI of the type from localId (replace module key after '/static/')
//   const makeTypeId = (typeModuleKey) => {
//     if (!localId || !typeModuleKey) return null;
//     const prefix = localId.split("/static/")[0] + "/static/";
//     return `${prefix}${typeModuleKey}`;
//   };



//   const handleTemplateConfigNext = async (configDataFromStep3) => {
//     try {
//       const typeModuleKey = TYPE_KEYS[selectedTemplate.name];
//       const typeId = makeTypeId(typeModuleKey);

//       console.log("typeModulekey", typeModuleKey);
//       console.log("typeid", typeId);
//       if (!typeId) {
//         alert("Could not calculate typeId for the custom field.");
//         return;
//       }
...
//         name: fieldName.trim(),
//         description: fieldDesc,
//         typeId,                      // <-- pass the ARI of your custom field TYPE
//         // config: { templateType: selectedTemplate.name }  // 👈 pass template info
//         // optional convenience:
//         // addToDefaultScreen: true,    // auto add to default screen
//         // you can also pass desired context params later if needed
//       });
//       if (!created || !created.id) {
//         throw new Error("Backend did not return a valid field ID");
//       }
...
//       // keep the created field id for Step 4 (ScreenAssociation)
//       setConfigData({ ...configDataFromStep3, customFieldId: created.id });
...
//     { name: "Linksy", desc: "Effortless Linking in Jira", category: "Picker" },
//     { name: "MoSCoW Prioritization", desc: "Prioritize using MoSCoW categories", category: "Prioritization" },
//     { name: "T-Shirt Size", desc: "Use T-Shirt size as a relative emitation technique in agile projects.", category: "Prioritization" },
//     { name: "Custom Number", desc: "Custom number input field", category: "Number" },
//     { name: "Other Field", desc: "Special custom field", category: "Other" }
//   ];
...
//         {step === 3 && selectedTemplate?.name === "T-Shirt Size" && (
//           <TShirtSizeField
//             onNext={handleTemplateConfigNext}
//             onClose={onClose}
//             fieldName={fieldName}
...
function ModalCustomField({ onClose, onSelect }) {
  ...
  const handleTemplateConfigNext = async (configDataFromStep3) => {
    try {
      ...
      // 🔑 persist the config against this fieldId
      await invoke("saveFieldConfig", {
        fieldId: created.id,
        config: configDataFromStep3, // contains viewType, permissions, etc.
      });
        ...
        {step === 2 && selectedTemplate && (
          <>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 16 }}>
              <button onClick={onClose}>Cancel</button>
              <button onClick={() => setStep(1)}>Back</button>
              <button onClick={handleStep2Next} style={{ background: "#0052cc", color: "#fff", border: "none", padding: "6px 12px", borderRadius: 4 }}>
                Next
              </button>
            </div>
          </>
